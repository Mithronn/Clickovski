  ...
  useEffect(() => {
    ...
    localforage
      ...
      .then(async (res: any) => {
        ...
        // Set Theme
        document.body.style.backgroundColor = JSON.parse(res)?.isDarkMode
          ? "rgba(32,28,28,1)"
          : "#ffffff";
        ...
        await getI18n().changeLanguage(
          JSON.parse(res || "{}").language || defaultStoreData.language
        );

        // watch startup plugin state and set a value to global store
        invoke("plugin:autostart|is_enabled").then((enabledState) => {
          ...
          if (enabledState) {
            localforage.setItem(
              ...
              JSON.stringify(
                res
                  ? { ...JSON.parse(res), isStartUp: true }
                  : { ...defaultStoreData, isStartUp: true }
              )
            );
          } else {
            localforage.setItem(
              ...
              JSON.stringify(
                res
                  ? { ...JSON.parse(res), isStartUp: false }
                  : { ...defaultStoreData, isStartUp: false }
              )
            );
          }
        });
    ...
    <I18nextProvider i18n={i18n}>
      <Provider store={store}>
        <ThemeProvider theme={theme}>
          <ModeThemeProvider enableSystem={false} attribute="class">
            {children}
          </ModeThemeProvider>
        </ThemeProvider>
      </Provider>
