...
import {
  setLanguage as setLanguageForRedux,
  setDarkMode as setDarkModeForRedux,
  setGlobalShortcut,
} from "@/redux/actions";
...
function Settings() {
  ...
  const [isActivatedSettings, setActivatedSettings] = React.useState<string[]>(
    []
  );
  ...
  const [isListenedKeys, setListenedKeys] = React.useState(
    reduxState.isGlobalShortcut
      ? String(reduxState.isGlobalShortcut).split("+")
      : []
  );
  ...
  const setLanguageFunction = async (item: any) => {
    ...
    localforage
      .getItem<string>("settings")
      .then((res) => {
        localforage.setItem(
          "settings",
          JSON.stringify({
            ...JSON.parse(res || defaultStoreData.toString()),
            language: item.name,
          })
        );
      })
      .catch((err) => {
        localforage.setItem(
          "settings",
          JSON.stringify({
            ...JSON.parse(defaultStoreData.toString()),
            language: item.name,
          })
        );
      });
  ...
  const setNotifications = () => {
    localforage
      ...
      .then((res) => {
        localforage.setItem(
          ...
          JSON.stringify({
            ...JSON.parse(res || defaultStoreData.toString()),
            isNotifications: isActivatedSettings.find(
              (x) => x === "isNotifications"
            )
              ? false
              : true,
    ...
    !!isActivatedSettings.find((x) => x === "isNotifications")
      ? setActivatedSettings((state) =>
          state.filter((x) => x !== "isNotifications")
        )
      : setActivatedSettings((state) => [...state, "isNotifications"]);
  };
    ...
    localforage
      ...
      .then((res) => {
        ...
        localforage.setItem(
          ...
          JSON.stringify({
            ...JSON.parse(res ?? defaultStoreData.toString()),
            isStartUp: startUpState ? false : true,
          })
        );
      })
      .catch((err) => {
        localforage.setItem(
          ...
          JSON.stringify({
            ...JSON.parse(defaultStoreData.toString()),
            isStartUp: isActivatedSettings.find((x) => x === "isStartUp")
              ? false
              : true,
          })
        );
      });
    !!isActivatedSettings.find((x) => x === "isStartUp")
      ? setActivatedSettings((state) => state.filter((x) => x !== "isStartUp"))
      : setActivatedSettings((state) => [...state, "isStartUp"]);
  };
  ...
  const setDarkModeFunc = () => {
    localforage
      ...
      .then((res) => {
        localforage.setItem(
          ...
          JSON.stringify({
            ...JSON.parse(res ?? defaultStoreData.toString()),
            isDarkMode: isActivatedSettings.find((x) => x === "isDarkMode")
              ? false
              : true,
          })
        );
        setTheme(
          isActivatedSettings.find((x) => x === "isDarkMode") ? "light" : "dark"
        );
      })
      .catch((err) => {
        localforage.setItem(
    ...
    setDarkMode(
      isActivatedSettings.find((x) => x === "isDarkMode") ? false : true
    );
    dispatch(
      setDarkModeForRedux(
        isActivatedSettings.find((x) => x === "isDarkMode") ? false : true
    ...
    isActivatedSettings.find((x) => x === "isDarkMode")
      ? setActivatedSettings((state) => state.filter((x) => x !== "isDarkMode"))
      : setActivatedSettings((state) => [...state, "isDarkMode"]);
  };
  ...
  React.useEffect(() => {
    localforage
      .getItem<string>("settings")
      .then((res) => {
        let datas = JSON.parse(res ?? defaultStoreData.toString());
  ...
  return (
    <div
      className={`max-h-[calc(100vh-64px)] min-h-[calc(100vh-64px)] p-6 pb-12 overflow-hidden ${
    ...
    >
      <div className="w-full relative h-full">
        ...
        <motion.p
          ...
          exit={{ opacity: 0 }}
          transition={transition}
          className={`font-Readex font-bold opacity-0 left-8 top-0 absolute p-1 select-none ${
            isDarkMode ? "text-white" : "text-black"
          } duration-150`}
        >
          {t("settings")}
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={transition}
        className={`opacity-0 border-t-2 ${
          isDarkMode
      ...
      >
        <div className="flex flex-col space-y-6">
          <p
            className={`font-Readex text-base select-none ${
              isDarkMode ? "text-white" : "text-black"
            } duration-150`}
          >
            {t("general")}
          ...
          <div className="flex flex-col space-y-5 pl-2 pr-2">
            <label className="flex flex-row justify-between items-center cursor-pointer">
              <p
                className={`font-Readex text-sm select-none cursor-pointer ${
                  isDarkMode ? "text-white" : "text-black"
                } duration-150`}
              >
                {t("start_up")}
            ...
            <div className="flex flex-row justify-between ">
              ...
              >
                <button
                  ...
                  className={`outline-none flex flex-row overflow-hidden justify-between items-center relative focus:outline-none w-1/2 rounded-md pr-4 pt-1 pb-1 font-Readex ${
                    isListeningKeyboard
                      ? "shadow-[#ff7070]"
                      : "shadow-[#ff707000]"
          ...
          <ClickAwayListener onClickAway={() => setLanguageOpen(false)}>
            <div className="relative flex md:max-w-[50%] lg:max-w-[60%] max-w-[100%]">
              <button
                ...
                className={`relative border border-transparent w-full flex flex-row space-x-3 items-center p-3 ${
                  isDarkMode ? "bg-darkestgray shadow-xl" : "bg-gray-200 shadow"
              ...
              >
                ...
                />
                <p
                  className={`font-Readex select-none ${
                    isDarkMode ? "text-white" : "text-black"
                  } duration-150`}
                >
                  {isLanguage}
              ...
              <AnimatePresence>
                {isLanguageOpen && (
                  <motion.div
                    ...
                    } rounded-lg outline-none focus:outline-none max-h-64 overflow-x-hidden overflow-y-auto ${
